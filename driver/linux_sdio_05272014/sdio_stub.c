...
extern void sw_mci_rescan_card(unsigned insert);
...
{
        esp_dbg(ESP_SHOW, "sw_mci_rescan_card %d\n", insert);
	sw_mci_rescan_card(insert);
}
...
void sif_platform_check_r1_ready(struct esp_pub *epub)
{
	...
	int err;
        ...
        func = sctrl->func;
        ASSERT(func != NULL);
	err = sw_mci_check_r1_ready(func->card->host, 1000);
        if (err != 0)
